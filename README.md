...
| CDP 浏览器操作   | 直连用户日常 Chrome，天然携带登录态，支持动态页面、交互操作、视频截帧            |
| 双模式 CDP       | CDP Proxy（curl API，精确 DOM 控制）+ agent-browser（snapshot/ref，AI 友好交互） |
...
### agent-browser（AI 友好交互）

当任务涉及复杂页面交互（多步表单、UI 走查、视觉审查）时，[agent-browser](https://github.com/vercel-labs/agent-browser) 的 snapshot/ref 模式更可靠——AI 直接从可访问性树中识别元素，无需手写 CSS 选择器。

```bash
npm i -g agent-browser                 # 安装
agent-browser close && agent-browser connect <port>  # 连接用户 Chrome
agent-browser open https://example.com && agent-browser snapshot -i  # 查看页面
agent-browser click @e1                # 通过 @ref 操作
```

详见 [SKILL.md](./SKILL.md) 中的 agent-browser 模式章节。
